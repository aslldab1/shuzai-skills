...
description: openclaw 协调 Claude 单 worker 开发循环。Claude 通过 Codex 插件调度 Codex 实现任务，以 GitHub Issues 为唯一任务数据库，Claude 负责设计/路由/review/验收/Codex 调度。
---
...
- `owner/claude` — 由 Claude 直接执行
- `owner/codex` — 由 Claude 通过 Codex 插件调度执行
...
- `【CODEX】` — Claude 代 Codex 写入（完成信号）

**完成信号**（openclaw 唯一的状态推进依据）：
- `【CLAUDE】【完成】PR #{pr_number} related to #{N}` — Claude 任务完成
- `【CODEX】【完成】PR #{pr_number} related to #{N}` — Codex 任务完成（由 Claude 通过插件执行后代写）
...
| Agent | 角色 | 职责 |
|-------|------|------|
| Claude | 主 worker（tmux 会话） | 路由、设计、review、验收、调度 Codex |
| Codex | Claude 的插件工具 | 实现、提 PR（由 Claude 通过 `/rescue` 或 `codex:codex-rescue` 调度） |

Claude 通过 Codex 插件调度任务时，Codex 在 Claude 进程内执行，使用 codex-bot 账户提交代码。

### Memory 持久化

Memory 存储在本地 JSON 文件，路径：`~/.openclaw/coding-team-loop/memory.json`
- 每轮 Step 1 最先读取，Step 4 最后写回
- 文件不存在（首次运行）→ 使用默认值
...
- **对每个 `in-progress` 且无 `epic` label 的 Issue，读取最近 10 条评论（只取 author/body/createdAt，控制数据量）：**
  > `epic` Issue 不直接执行，由 Step 1.6 子任务联动驱动，跳过完成信号和 stale 检测。
  ```bash
  ...
  - 检测完成信号：最新评论**精确包含** `【CLAUDE】【完成】` 或 `【CODEX】【完成】` 字样（不含`【完成】`的评论一律忽略）
    ...
    - **无 owner label（P8 路由任务）完成** → 解析 【CLAUDE】【完成】 评论内容：
      - 含 `路由决策: owner/claude` 或 `路由决策: owner/codex` → 添加对应 owner label，移除 `in-progress`，加回 `pending`（下轮 P6/P7 正常派发）
      - 含 `已拆解为 epic` → 添加 `epic` label，移除 `in-progress`（子 Issue 已带 `pending` + owner label，下轮自动派发）
      - 评论格式不符 → 留 【OPENCLAW】 评论要求重新提交路由决策，不做状态变更
- **对每个 `verifying + blocked` Issue：** 查询所有 body 含 `fix for #{此Issue编号}` 的 Issue（open + closed）
  ...
  - 检测 stale：无完成信号 + 最后活动时间（最新评论或分支最新 commit）距今 > 20 分钟
    - → 标记为"待确认"，Step 2 发送进度确认消息（→ 需要时读取：refs/progress-confirm.md）
- **对每个 `pending` + `owner/claude` 或 `owner/codex` 的 Issue，额外读取最近 10 条评论，供 Step 2 派发判断使用：**
  ```bash
  gh issue view {N} --repo {REPO} --json comments --jq '.comments[-10:] | map({author:.author.login,body:.body,createdAt:.createdAt})'
  ```
...
按优先级选择第一个匹配（**所有优先级均自动排除 `epic` Issue**，epic 不直接执行，由 Step 1.6 子任务联动驱动）：
...
**P3** — 有 `changes-requested` Issue
→ 派发修复请求给 Claude，由 Claude 通过 Codex 插件调度 Codex 修复
...
**P5** — 有 `in-progress` + stale（Step 1 标记为"待确认"，任意 owner）
→ 发送进度确认消息给 Claude
→ 对 `owner/codex` 的 stale 任务，Claude 通过 Codex 插件检查任务状态
→ 需要时读取：refs/progress-confirm.md

**P6** — 有 `pending` + `owner/claude` Issue
→ 取编号最小的一条
→ **读取该 Issue 完整评论历史（Step 1 已读）+ Claude pane 内容，综合判断当前状态后派发**
→ 需要时读取：refs/task-dispatch.md

**P7** — 有 `pending` + `owner/codex` Issue
→ 取编号最小的一条
→ 派发给 Claude，由 Claude 通过 Codex 插件调度执行
→ **读取该 Issue 完整评论历史（Step 1 已读）+ Claude pane 内容，综合判断当前状态后派发**
→ 需要时读取：refs/task-dispatch.md

**P8** — 有 `pending` + 无 owner label 的 Issue
→ 派发路由请求，Claude 判断 owner 或拆解 epic
→ 需要时读取：refs/task-dispatch.md（"P8 路由消息格式"部分）
→ Claude 完成后必须写 【CLAUDE】【完成】 信号（Step 1 据此推进状态）
...
> **父 Issue 保持 `in-progress`，不改状态。** 下一轮 Step 2-P7 会正常找到新 Issue 并派发给 Claude（由 Claude 通过 Codex 插件执行）。
...
workers:
  ...
  codex:
    last_dispatched_at: ""      # 通过 Claude 插件调度的 Codex 任务
    last_issue_number: null

...
## 判断规则（Claude 和 Codex 通用）

只判断一个信号：**pane 底部是否有忙碌行首符号**。有 → 忙碌，没有 → 空闲。

### 忙碌行首符号

Worker 工作时，状态行以特定符号开头（完成后被结果替换，不会留在历史中）：

| 符号 | Worker | 示例 |
|------|--------|------|
| `✢` `✦` `✳` `✶` `✻` `✽` | Claude Code（6 个 dingbat 符号轮转） | `✢ Generating… (49s · ↓ 87 tokens)` |
| `•` + 动名词 | Codex | `• Working (5s • esc to interrupt)` |
| `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏` | 通用 spinner | 旋转进度指示 |
...
**Codex 忙碌时：**
```
• Working (5s • esc to interrupt)       ← 行首 •
```
...
- 忙碌时本侧步骤跳过，不发送任何消息，等下一轮
- 若 pane 不存在，视为 worker 会话缺失 → 参考 session-recovery.md
